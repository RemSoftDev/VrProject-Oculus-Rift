...
namespace ComPortPackages.Console
{
    public class ComPrortSender
    {
        ...
        public static void Send(string pathToMotionFile,TextBox textBox_Terminal,Application app,Action<string> callbackMessage, Action<Exception> callbackException)
        {
            App = app;
            CallBacker.CallBackMessage = null;
            CallBacker.CallBackException = null;
            CallBacker.CallBackMessage = callbackMessage;
            CallBacker.CallBackException = callbackException;
            try
            {
                ...
                //{
                  ...
                  , package);
                    ...
                    comPortPackagesService.Completed += (s, e) =>
                    {
                        ...
                        CallBacker.CallBackMessage?.Invoke("Отправка данных закончена");

                        if (string.IsNullOrWhiteSpace(package.ProccessName) == false)
                        {
                            CallBacker.CallBackMessage?.Invoke($"Остановка процесса {package.ProccessName} ...");
                            var process = Process.GetProcessesByName(package.ProccessName).FirstOrDefault();
                            if (process == null)
                            {
                                CallBacker.CallBackMessage?.Invoke($"Процесс {package.ProccessName} не найден в системе");
                            }
                            else
                            {
                                process.Kill();
                                CallBacker.CallBackMessage?.Invoke($"Процесс {package.ProccessName} успешно остановлен");
                            }
                        }
